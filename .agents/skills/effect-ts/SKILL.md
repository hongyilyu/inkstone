...
Before starting any Effect-related work, verify the Effect-TS source code exists at `~/dev/effect`.

**If missing, stop immediately and inform the user.** Clone it before proceeding:

```bash
git clone https://github.com/Effect-TS/effect.git ~/dev/effect
```
...
2. **Effect Source Code** — For complex type errors, unclear behavior, or implementation details, examine the Effect
   source at `~/dev/effect/packages/effect/src/`. This contains the core Effect logic and modules.
...
Effect.all([..dev/effects])        // Run effects (concurrency configurable)
...
- **`~/dev/effect/packages/effect/src/`** — Core Effect modules and implementation
