...
import { VAULT_DIR, ARTICLES_DIR, NOTES_DIR, SCRAPS_DIR } from "./constants"
...
export function setConfirmFn(fn: (title: string, message: string) => Promise<boolean>) {
  confirmFn = fn
}
...
export async function beforeToolCall(
  ...
  if (articlePath && targetPath === articlePath) {
    ...
    if (toolName === "write_file") {
      return {
        block: true,
        reason: "Cannot overwrite the article file. Use edit_file to modify frontmatter only.",
      }
    }
    ...
    if (toolName === "edit_file") {
      ...
      if (!frontmatter || !frontmatter.includes(args.oldText)) {
        return {
          block: true,
          reason: "Only frontmatter modifications are allowed on the article file.",
        }
      }
    }
  ...
  if (toolName === "write_file" || toolName === "edit_file") {
    if (targetPath.startsWith(NOTES_DIR) || targetPath.startsWith(SCRAPS_DIR)) {
      if (confirmFn) {
        const ok = await confirmFn("Write confirmation", `Allow write to ${targetPath}?`)
