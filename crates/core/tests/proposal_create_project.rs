//! An accepted `create_project` Proposal validates a `ProjectData` payload and
//! persists a `project` Entity sourced `created_from` the user Message (ADR-0031,
//! ADR-0025). A `{name}`-only payload defaults `status` to `active` in the stored
//! entity data — the headline assertion of this slice.
//!
...
#[test]
fn accept_create_project_defaults_status_and_resumes() {
    let workspace = Workspace::new();

    // Point the fixture at a `create_project` mutation via a tempfile. The
    // payload carries only `name`; the apply path must default `status` to
    // `active` in the stored entity data.
    let params_dir = tempfile::Builder::new()
        .prefix("inkstone-create-project-")
        .tempdir()
    ...
    let (run_id, entity_id) = rt.block_on(async {
        let run_id = create_and_park(&core, "Start the API v2 migration outcome.").await.0;
    ...
    rt.block_on(async {
        ...
        // One Project entity, created via the proposal.
        let row = sqlx::query(
        ...
        assert_eq!(etype, "project", "entity type is project");
        ...
        assert_eq!(
            ...
            Some("Ship API v2 migration"),
            "entity data round-trips name — got {data}"
        );
        // Headline: a {name}-only create defaults status to active in stored data.
        assert_eq!(
            data_json["status"].as_str(),
            Some("active"),
            "stored project data defaults status to active — got {data}"
        );
        ...
        assert!(
            ...
            "Project sourced from a Message has NULL source_entity_id"
        );
