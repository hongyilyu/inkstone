...
describe("command palette", () => {
	...
	test("agent-declared verbs do not appear in the palette", async () => {
		...
		// in the slash dropdown only — they're irrelevant to palette
		// browsing because they require the user to be in (or commit
		// to) the owning agent. Default fake session binds to Reader,
		// so `/article` would appear today; pin it out.
		const fake = makeFakeSession();
		setup = await renderApp({ session: fake.factory });
		await setup.renderOnce();
