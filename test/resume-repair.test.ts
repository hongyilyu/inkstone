...
describe("loadSession — tail repair", () => {
	...
	test("clean tail (user + assistant) → no synthetic appended", () => {
		const sid = seedSession([userMsg("hello"), assistantMsg()]);
		const loaded = loadSession(sid);
		expect(loaded).not.toBeNull();
		const msgs = loaded!.agentMessages;
		expect(msgs.length).toBe(2);
	...
	test("dangling tail with prior assistant → placeholder inherits api/provider/model", () => {
	...
	test("dangling user on first turn (no prior assistant) → placeholder uses dummy metadata", () => {
		const sid = seedSession([userMsg(":q")]);
		const loaded = loadSession(sid);
	...
	test("interior gap (user, user, assistant) → placeholder inserted between users", () => {
		...
		const sid = seedSession([userMsg(":q"), userMsg("test"), assistantMsg()]);
		const loaded = loadSession(sid);
		const msgs = loaded!.agentMessages;
		expect(msgs.length).toBe(4);
		expect(msgs[0]?.role).toBe("user");
	...
	test("tool-result row between users → repair triggers on last user/assistant role", () => {
	...
	test("sequential dangling gaps → second placeholder doesn't inherit placeholder metadata", () => {
