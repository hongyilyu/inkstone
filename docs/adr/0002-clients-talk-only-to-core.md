...
Every Client (Web, TUI, Desktop, Mobile, Capture Client) talks to Core through Core's client surface. Clients do not access the SQLite database, the Worker, or LLM providers directly.

## Why

Core is the coordination authority for a Workspace: it owns durable state, enforces invariants on the SQLite database, mediates Run lifecycle, and brokers credentials. Letting any Client bypass it splits authority — two Clients editing the database concurrently, or holding their own LLM keys, would defeat Core's role and produce inconsistencies that are expensive to recover from.
