...
- **Vault file ingestion bookkeeping** (`vault_files`, `snapshots`, `ingestion_log`). Slice 1 has no watcher; deferred to the slice that adds external-editor capture. Vault-write Proposals are likewise out of scope: the `proposals.kind + payload JSON` polymorphism can carry them when they land, but no schema target exists in slice 1 because [ADR-0010](./0010-mvp-slice-chat-driven-web-client.md) excludes Vault writes from this slice.
