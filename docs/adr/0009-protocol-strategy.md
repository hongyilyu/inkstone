...
The contract-test leg this ADR called for now exists in `tests/contract`, and it covers a second, larger surface beyond the Worker ↔ Core protocol types this ADR was written about: the **agent-proposable Workspace mutation payloads**. Core single-sources each of the 15 proposable kinds' payload shape from one `PayloadSpec` (`crates/core/src/field_spec.rs`), which emits an inline Draft-07 fragment; the Web side hand-mirrors each as an Effect Schema. The gate dumps the Rust schema per kind to a committed fixture and asserts the Effect Schema, run through `JSONSchema.make` and normalized to a common dialect, deep-equals it. The wire `payload` itself stays `S.Unknown` in `packages/protocol` — this is a contract *test*, not a wire-type change.

**Update (2026-06): the 15 schemas live in `packages/protocol`, not the test package.** The hand-mirrored Effect Schemas were promoted from `tests/contract/src/schemas.ts` into `packages/protocol/src/payloads.ts` (re-exported from the package barrel), and `tests/contract` now imports the registry from `@inkstone/protocol`. The move is what makes the parity gate guard the *shipped* schema rather than a test-only mirror: the same `schemas` object the Web's per-Entity-Type codec (`apps/web/src/lib/entityCodec.ts`) decodes/encodes against is the one the gate deep-equals with the Rust fixtures. The wire boundary is unchanged — `EntityMutateParams.payload` / `EntityRow.data` stay `S.Unknown`; the typed schemas are a Web-side validation concern layered over the opaque wire, not a tightening of the protocol type. The 15-kind `schemas` registry the parity gate iterates is unchanged in membership; `completeness.test.ts` still locks it to exactly the 15 committed fixtures.

`packages/protocol` additionally holds three **ungated** media payload schemas (`createMedia` / `updateMedia` / `deleteMedia`) the Web codec consumes for the `media` editor (the queue+log Entity Type, ADR-0059, which replaced the earlier `bookmark` kind). These are deliberately **outside** the 15-kind parity registry and have no Rust fixture, because `media` is a user-CRUD-only kind the agent never proposes (ADR-0059) — Core validates it but does not advertise it via `PayloadSpec`, so there is no Rust schema to reconcile against. The trade-off is explicit: these three schemas can drift from Core's `validate_media` without the gate catching it, the same structural-drift exposure the gate otherwise closes. They earn their place by giving the Web codec one uniform schema-backed shape across all five Library editors; if `media` ever becomes agent-proposable, it gains a `PayloadSpec` + fixture and joins the gated registry.
...
- **The worker union is an exact mirror.** TS `WorkerOutbound = WorkerRunEvent | ToolRequest` mirrors Rust's `WorkerStdout` one-for-one; the authored `WorkerStdout` fixtures decode against it. *(Originally the TS side reused `RunEvent`, leaving `WorkerOutbound` deliberately broader — the Worker never emits `RunEvent`'s Core-synthesized `cancelled`/`tool_call` — with the asymmetry documented at the test. The `WorkerRunEvent` narrowing removed it.)*
...
## As-built: read-data schema coverage (2026-06)

Both gate legs above guard what crosses the wire *as a typed shape*. The per-Entity-Type **`data` blob** was the remaining blind spot: `EntityRow.data` ships as an opaque `serde_json::Value` (Rust) / `S.Unknown` (TS), and the Web codec (`apps/web/src/lib/entityCodec.ts`) reconstructed each Entity Type's stored fields by hand, with drift caught by nothing — a `*_core` field add or rename on the write side silently went un-parsed on read. This closes that gap; it extends the existing Web-side validation layer, it is **not** a wire-type change (`EntityRow.data` stays `S.Unknown`) and not a new decision.

The mechanism reuses the payload gate rather than adding a Rust leg. The stored `data` field-set is identical to the inner `*_core` `PayloadSpec` the write payloads already advertise (`person_core`/`project_core`/`media_core`/`todo_core`, `crates/core/src/mutation.rs`), so the existing payload fixtures (`fixtures/create_person.json`, etc.) already pin those fields. The Web side authors a **relaxed read schema** per Entity Type in `packages/protocol/src/payloads.ts` (built from the same shared leaf-builders as the write schemas), and a TS-only parity test (`packages/protocol/src/*.test.ts`) asserts the read schema is a strict **superset** of the write schema's field-set: it accepts everything the write side emits, *plus* tolerates the missing / empty / legacy values the read path must accept (a stored Todo with no `title`, a `review_every` Core stored before the current shape, etc.). The codec then decodes `row.data` against the read schema and hand-assembles the camelCase view model, keeping its imperative coercion (default-on-missing, coerce-wrong-type, drop-partial) for the genuinely tolerant cases.

Boundaries (they bound what "the read data agrees" means):

- **Leverage is Todo/Person/Project only.** Those three derive their read shape from a write `*_core`, so a Rust field add reds the payload fixture → forces the write schema → reds the superset test → forces the read schema. JournalEntry has no write-*data* sibling (its `journal_entry_payload` models the write-only `body`/`target` input, `mutation.rs`), and Media is deliberately ungated (ADR-0059, user-CRUD-only) — both get hand-authored read schemas with no Rust-fixture pin, the same coverage they had before. Not a regression; the existing gate boundary.
- **The read schema is strictly looser than the write schema, by design.** It is *not* `S.partial(write)` — a partial keeps the write side's `nonEmptyString` and would reject a sparse stored `title`. The superset test makes that asymmetry (lenient ingest, strict emit) machine-checked rather than reviewer-trusted — the same asymmetry-with-a-test discipline `openai/codex`'s `app-server-protocol` enforces for its Rust↔TS protocol (a CI test gates `#[ts(optional=nullable)]` to input types only).
- **Migration target unchanged.** If ADR-0009's revisit triggers fire, `openai/codex`'s `app-server-protocol` (one Rust type → `ts-rs` TS bindings + `schemars` JSON Schema + a regenerate-and-diff fixture test) is a working template for the codegen path this ADR declined — structurally the same Rust-core-plus-TS seam, parity by construction instead of by hand-mirror.
