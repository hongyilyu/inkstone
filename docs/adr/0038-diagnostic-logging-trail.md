...
## `run_id` travels in-band on the `WorkerManifest`

`run_id` is a **required field on `WorkerManifest`** (`packages/protocol/src/index.ts` and its mirrored Rust struct, [ADR-0009](./0009-protocol-strategy.md) manual mirror), so it rides the same schema-validated channel as the work itself. `transport-stdio.ts` no longer hardcodes `run_id: ""` in outbound frames, and Core no longer sets `INKSTONE_RUN_ID`. For `worker.jsonl` lines to join to `core.jsonl` by run, the Worker reads `run_id` from the manifest it already parses and stamps each line.

Identity now travels in-band on the structured manifest the Worker already validates, so there is no second, unvalidated channel and no way for the correlation key the trail exists to provide to silently go empty.
...
- **Acknowledged gaps, not fixed here:** (1) a hard Worker crash *before* it can write JSON (uncaught top-level throw) still goes to inherited stderr and vanishes — the cost of the simplest seam, since `child.rs` keeps `Stdio::inherit()`; (2) the Web Client emits nothing — client-side capture is out of scope for v1.
...
- **`run_id` in `WorkerManifest` now (in-band).** Rejected *for v1* to keep the feature contract-free — it is a `packages/protocol` contract change rippling protocol→core→worker for a field the Worker uses only to label log lines — but adopted immediately after in [issue #146](https://github.com/hongyilyu/inkstone/issues/146), which moved `run_id` in-band and retired the spawn env var.
...
- [ADR-0013](./0013-worker-process-lifecycle-and-transport.md) — the Worker child-process lifecycle (`child.rs`, `Stdio::inherit()`) whose `Stdio::inherit()` seam is why hard crashes can still escape capture.
...
- [issue #146](https://github.com/hongyilyu/inkstone/issues/146) — moved `run_id` from the spawn env var into `WorkerManifest` (in-band), retiring the out-of-band seam this ADR originally shipped.
