...
- **The original Proposal implementation handled only entity creation.** The first journal-capture extension replaces `propose_entity` with `propose_workspace_mutation`: the durable Proposal row stores a closed `mutation_kind` (for example `create_journal_entry`) rather than overloading an Entity Type `kind`, and `change_kind` is no longer part of the first journal-capture contract. The park/resume mechanism in this ADR is unchanged.
- **The decide transaction is a deep, testable module (refines [ADR-0029](./0029-request-handler-seam.md)).** The idempotency / guarded apply-reject / resume-and-still-parked-recovery described above lives in `crate::decide::apply`, returning a typed `DecideOutcome`/`DecideError`; `runs/proposal.rs::handle_decide` is a thin JSON-RPC shell over it, with `worker::resume` injected as a closure so the whole path (accept, reject, edit, same-key replay, lost race, still-parked recovery) is asserted as a value against a `:memory:` pool. Entity-Type dispatch (validate, schema version, accept-decision text) sits behind the `entities` module so `decide` and the db layer never match on the Entity Type — readying the type→validator generalization for when Person/Project land. One ordering subtlety is load-bearing: the `edit`-requires-`edited_payload` check sits on the **fresh apply path** (after the idempotency-replay, the already-decided recovery, and the run-parked gate), **not** at decode/parse time. A payload-less `edit` *retry* of an already-decided, still-`parked` Proposal must therefore recover (return the prior result + re-drive resume) rather than fail `invalid_params` and wedge the Run; only a *fresh* `edit` missing its payload is `invalid_params`. (Hoisting that check ahead of the idempotency/recovery branches is the one regression that surfaced during implementation.)
...
- [ADR-0018](./0018-workflow-and-tools-definition.md) — Proposal tools are Core-registered tools; the manifest gains typed message blocks + a `mode: fresh|resume` field here.
