...
The Client prioritizes the existing entity: a `create` node with **exactly one** `near_match` **defaults to reuse it** — the node commits with the per-node `entity_id` override pointing at that entity (badge "Existing «Lead Ads»"), and a **"Create new instead"** escape lets the user mint a fresh entity when that is genuinely intended. A node with **two or more** near-matches surfaces them advisorily but does **not** auto-pick — picking among a *create* node's near-matches is not yet built (distinct from the `ambiguous`-candidate picker, which shipped per the as-built note above); it stays `create`.
...
>
> **The carve-out: anchor-reuse.** The `journal_entry` node gains an optional `existing_id`. When present, the node is **anchor-reuse**, the one disposition the JE node can carry: Core mints **no** new Journal Entry and resolves the node to that existing JE id. The graph still mints the newly-recognized People/Projects/Todos and weaves `journal_ref` body mentions + `EntityRef` rows — but those mentions are spliced into the **stored** body of the existing JE (a new revision), not a newborn one. Every other JE node (no `existing_id`) stays create-only exactly as above. A re-scan graph always has **≥1 create** (a pure all-reuse graph is still degenerate `InvalidMutation`); adding only a link between two already-existing entities is out of scope (the cross-thread search picker, #181, is needed for the general case).
>
> **Stored-body splice, not re-emit.** In anchor-reuse mode the model emits **no body**. Core reads the JE's **stored** body and splices in only the new chips, leaving existing `entity_ref` nodes and their `ref_id`s untouched (so no dangling refs and no re-mint). Placement comes from a new optional **`match_text`** on each `journal_ref` link: the exact substring the model recognized (`"Priya"`). Core finds the **first un-chipped occurrence** of `match_text` in the stored body's text nodes and splits it `[text][entity_ref][text]`. A `match_text` not found in the stored body → `InvalidMutation` (the prose-faithfulness guard). Because Core only **splits** a stored text node and never edits characters, prose drift is impossible by construction — the model cannot invent or reword the user's record. Pulling an entity from the *surrounding conversation* (text never in the JE) is therefore impossible here and is filed separately ([#221](https://github.com/hongyilyu/inkstone/issues/221)).
>
> **Validation relaxes for anchor-reuse.** `validate_body_targets` (which requires every body `entity_ref` placeholder to have a matching `journal_ref` link) is keyed to a model-emitted body, so it no longer applies when the body is absent; instead each `journal_ref` link in anchor-reuse mode carries `match_text` and Core validates that substring against the stored body at apply.
>
> **Cross-thread guard is now load-bearing (same-thread enforcement).** Because anchor-reuse **updates** an existing JE rather than minting a newborn one, the ADR-0030 cross-thread guard (`journal_entry_target_is_valid`) now gates this graph: it passes only when the target JE's origin user-Message is in the re-scan Run's thread. In v1 that always holds (the re-scan Run is started in the JE's origin thread by `journal_entry/rescan`), and the guard is precisely the mechanism that **structurally** keeps v1 same-thread — a cross-thread re-scan fails the guard. The general cross-thread case stays deferred (it needs the unbuilt search/disambiguation picker beyond #181).
>
> **Trigger + suppression.** Re-scan is an explicit **"Scan again for missed entities"** action on the Journal Entry view (available on any accepted JE). It calls a new `journal_entry/rescan {je_id}` RPC that resolves the JE's origin thread and starts an ordinary Run there (reusing `default.toml`, taught the re-scan recognition pattern). Suppression is layered: Core's exact-match resolver stays the authority (an already-anchored or pool-existing entity resolves to `reuse`, mints nothing; the `EntityRef` weave is `ON CONFLICT(source,target) DO NOTHING`, so re-anchoring is a free no-op), and the re-scan prompt is additionally fed the JE's already-anchored entity names as an advisory "only surface new ones." A re-scan that finds nothing new proposes nothing — the model just replies conversationally, no empty card.
>
> The #181 picker does **not** gate same-thread re-scan: the picker fires on `ambiguous` (≥2 exact matches), whereas re-scan's suppression is the single-exact `reuse` case; near-twins ride the near-match advisory amendment below.

> **As-built amendment — anchor-reuse may APPEND a model-proposed clause (#221).** This reverses the **"Stored-body splice, not re-emit"** deferral above (the final sentence of line 163): folding in an entity mentioned only in the *surrounding conversation* — never in the JE's own prose — is now in scope for the **same-thread** case. The motivating flow is conversational, not a re-scan button: while still in the JE's origin thread the user adds a later fact ("oh, Priya was there too"), and the agent proposes folding Priya into the saved entry in the **one** reviewed proposal it already uses — no `create_todo`-then-`reference` two-step, no new web affordance.
>
> **Why splice-only was insufficient.** Anchor-reuse places a chip by `match_text` — the exact substring of the **stored** body the model recognized — and a `match_text` absent from that body is rejected (the prose-faithfulness guard). That is correct when the name is already written down, but it makes a later-mentioned entity **unplaceable**: its name is in the *thread*, not the *entry*. The shipped re-scan Run already runs in the JE's origin thread with same-thread history in context (`runs/journal_entry.rs`), so the model can *see* the later mention — it simply had no way to *place* a chip for it.
>
> **The carve-out: `append_text`.** A `journal_ref` link gains a second optional placement field, **`append_text`** — a short clause the model proposes (e.g. `"Followed up with Priya."`). Exactly one of `match_text` / `append_text` is set per `journal_ref` (both-set or neither-set → `InvalidMutation`). `match_text` present → splice into existing prose, byte-identical, exactly as before. `append_text` present → Core appends a new `{type:text, text:<append_text>}` node to the stored body and splices the chip **within that appended node** (reusing the same `splice_entity_ref_into_body` against the freshly-pushed text, so the chip-placement and minLength:1 guarantees are identical). The result is one new revision of the existing JE; no new `entities` row; the entity is minted + backlinked exactly as the splice path already does.
>
> **The prose-faithfulness relaxation, stated plainly.** `append_text` is a deliberate, narrow relaxation of "Core never invents prose" (line 163, `splice_entity_ref_into_body` doc). It is acceptable **only** because the appended text is *model-proposed and lands inside the same human-approved proposal the user already reviews* — the user sees the new sentence on the card before accepting, and rejecting the node drops the clause entirely (nothing is appended for a rejected `journal_ref`). Core never appends prose the user did not approve. `match_text` (splice-existing) remains the zero-prose-invention path and stays the default the recognition prompt prefers when the name is already written.
>
> **Everything else is reused unchanged.** Entity minting via the create loop, the cross-thread guard (`journal_entry_target_is_valid` — `append_text` is still same-thread-only, the general cross-thread case stays deferred), the `EntityRef` `ON CONFLICT(source,target) DO NOTHING` idempotency + authoritative-id read-back, the new-revision write, and `validate_woven_journal_body` (which validates the appended nodes too) all carry over. This is a **placement-branch** extension on `journal_ref`, not a new apply path — and it deliberately does **not** touch `update_journal_entry`, whose body refs must already belong to the JE and which therefore cannot mint (routing #221 there would reopen the #179 two-step this ADR killed).
