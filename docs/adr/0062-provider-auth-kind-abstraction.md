...
2. **`resolve_access_token` dispatches by auth kind, not by hardcoded provider id.**
   ...
   (As-built amendment: Run-creation sites now gate on `credentials::is_connected` *before* persisting the Run — a missing credential rejects with `-32004` `ProviderNotConnected`; a present-but-unparseable credential store rejects with `-32603` internal (fail loud, never a misleading "not connected"). Both write zero rows. The gate lives in `crate::start_run::ensure_provider_connected`; `resolve_access_token`'s `None` branch above remains the Worker-side fallback for tokens that expire mid-flight.)
