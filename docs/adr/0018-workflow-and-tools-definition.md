...
> - **Unbuilt fields.** `auto_approve` and `bootstrap` were sketched below but never landed in the Workflow data shape or the manifest. There is no auto-approve seam today — every Proposal parks the Run per ADR-0025; a future auto-approve policy would live at the decide layer per ADR-0016. No bootstrap tool-call list ships. `tools` ships populated.
