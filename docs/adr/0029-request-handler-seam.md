...
Both still benefit from the typed-id decode: their params are `Uuid`-typed like the rest, so a malformed id fails at decode. Their bodies stay hand-written.
