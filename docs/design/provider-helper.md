...
Core spawns it via the `INKSTONE_PROVIDER_HELPER_CMD` (refresh) / `INKSTONE_PROVIDER_LOGIN_CMD` (login) command-string seam; the default points at `packages/provider-helper/node_modules/.bin/tsx packages/provider-helper/src/provider.ts {mode}`. Tests point the env overrides at the standalone stubs `crates/core/tests/fixtures/{refresh,login}-helper.ts`.
