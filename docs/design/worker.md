...
- `INKSTONE_FAUX_PROPOSE === "1"` → scenario-driven ordered Turns via `INKSTONE_FAUX_PROPOSE_PARAMS` (required, fresh AND resume — same fail-fast shape as EXTRACT/CAPTURE); playback by manifest position (the count of user messages); no prompt parsing. Each Turn (`{action: create|update|delete, body?, occurred_at?}`) yields one `propose_workspace_mutation` (fresh) — update/delete resolve the entry via a real `read_current_thread_journal_entries` round-trip, entry[0] (or a text-only reply if the thread has no entry); omitted update fields keep the live entry's values — then Core persists a pending Proposal and PARKS (tearing this Worker down), the ADR-0025 park/resume dance. On resume (`mode:"resume"`) Core re-spawns with the reconstructed transcript ending in the Decision tool_result; the scenario is loaded/validated but its Turns aren't consumed — the loop continues with a short confirmation from that tool_result. The faux provider state is per-process, so the resume spawn freshly applies the resume response (mirrors propose-worker.ts at the protocol level).
