...
| Delete behavior | Phase 3, structural in #277 | Block deleting a Habit while `habit.checkin` rows reference it. | Now structural: the delete-block dispatches off `mutation::OBSERVATION_RELATIONS`, so any relation-bearing descriptor auto-blocks its target's delete (live + historical revisions) — no per-schema edit. A future cascade/allow policy would need a `delete_behavior` field on the descriptor. |
| Query surface | Phase 1, expanded in Phase 3, structural in #277 | `observation/query` supports schema/time/source/limit; Phase 3 adds `related_entity_id` (now descriptor-driven via `OBSERVATION_RELATIONS`). | Aggregates, buckets, cursors, and richer filters wait for real UI pressure. |
