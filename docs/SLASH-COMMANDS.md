...
- **Aliases: implemented (shell-level only).** `SlashSpec.aliases?: string[]` lets a single entry resolve from multiple typed verbs — e.g. `/clear` declares `aliases: ["new"]` so `/new` dispatches the same handler. The dropdown row stays canonical (`/clear`) — aliases widen the lookup AND fuzzysort match keys but never add extra rows. Resolution: `findSlashEntry` (`src/tui/components/dialog/command.tsx`) does a two-pass `name`-first / `aliases`-second match, so a canonical name always beats an alias on collision. Backend `AgentCommand` does NOT carry `aliases` — agent verbs stay alias-less by design (narrower surface; can opt in later if a real need emerges). Pinned by `test/command-slash.test.ts → describe("findSlashEntry")`.
- **Slash dropdown fuzzysort keys are `display` + `aliases` only.** Description text is intentionally NOT a search key. With description as a key, typing `/new` (an alias of `/clear`) would also surface `/ingest` ("Process **new** 010 RAW/ sources...") and `/query` (description fuzzy-subsequences `new`), drowning the canonical row in noise. Diverges from OpenCode's three-key shape (`autocomplete.tsx:406-411` keys on `display` + `description` + `aliases`) — Inkstone's smaller registry doesn't need description as a discovery vector, and aliases as a first-class field already cover the discoverability OpenCode bought via description-keying. Mention mode is unaffected — its rows have no description anyway. Regression-guarded by `test/tui/autocomplete.test.tsx`.
