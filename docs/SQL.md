...
| Table | Purpose |
|---|---|
| `sessions` | Conversation root. `agent` column scopes all reads. `title` is `TEXT NOT NULL`: new rows start with a human-readable default (`"New session - <ISO timestamp>"`), then background title generation replaces it after the first user prompt. `/clear` ends the in-memory session; the row stays on disk untouched. `parent_session_id` is a nullable lineage pointer set by `forkSession()` only — **NOT an enforced FK** (drizzle's self-reference syntax broke at runtime, so the column is plain TEXT). See ADR 0014. |
