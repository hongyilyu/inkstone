...
The prompt `<input>` is **uncontrolled** (no `value=` prop). Writes go through the renderable imperatively (`input.setText`, `input.insertText`, `input.deleteRange`); the JS signal `text()` mirrors `input.plainText` via `onInput`. This is a direct consequence of extmark semantics: `EditBuffer.setText` clears all extmarks (`ExtmarksController.wrapSetText` in `@opentui/core`), so a controlled input that writes `value=` back to the renderable on every reactive update would nuke mentions on every keystroke. OpenCode solves the same problem the same way. The slash dropdown's selection path (which previously used `props.setText`) migrated to imperative `input.setText` in the same change — forced by the uncontrolled switch. `clearInput()` centralizes the submit/clear path (`input.setText("")` + `setText("")`); the buffer's own `setText` call clears extmarks as a side effect, so no separate `extmarks.clear()` is needed.

Vault file list comes from `listVaultFiles()` (`src/tui/util/vault-files.ts`) — synchronous recursive walker, module-local lazy cache (first `@` trigger walks; subsequent triggers hit the cache). Skips leading-dot entries, `node_modules/`, symlinks (matching `readFileSafe`'s reject so the dropdown never surfaces a file the submit path would fail on). `invalidateVaultFileCache()` is exported but currently has no callers — reserved for a future "refresh" command or `fs.watch` hook.
