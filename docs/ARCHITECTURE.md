...
`DisplayMessage` carries `id`, `role` (`"user" | "assistant"`), `parts: DisplayPart[]` (ordered blocks), and optional assistant-only fields: `agentName`, `modelName` (set in `message_end`), `duration` (ms, set in `agent_end`), `error` (pi-ai `errorMessage` on `stopReason === "error"` only), `interrupted` (`true` on `stopReason === "aborted"`). `DisplayPart` is a discriminated union: `text` (with `text: string`), `thinking` (with `text: string`), and display-only `file` (with `mime` + `filename`).
