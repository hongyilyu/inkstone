...
Today's TUI closure extracts `title` + `message` and routes through `DialogConfirm` — the modal UI itself hasn't changed. In phase 4 the closure also writes the diff into a per-`callId` **preview registry** (`src/tui/context/agent/preview-registry.ts`, ephemeral `Map<callId, PendingPreview>` wrapped in a Solid signal) so that `ToolPart` can render the unified diff inline below the args line via OpenTUI's `<diff>` renderable. The registry is scoped to `AgentProvider`, cleared on unmount, and wiped per-`callId` when the approval resolves (regardless of approve/reject). The overlay avoids widening `DisplayPart` with a `diff` field that would leak into SQLite; the registry is a live-session-only view on top of persisted parts. Phase 5 (tracked in TODO) swaps the modal for a bottom panel — the preview registry survives that swap unchanged.
