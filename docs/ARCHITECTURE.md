...
3. `queueMicrotask`s into the existing `resumeSession` flow so the swap lands AFTER pi-agent-core's natural tail (`message_end` + `agent_end` after `terminate: true`) settles.

The TUI conversation renderer detects fork-marker rows (`role === "assistant"` + single `parts.type === "fork"` part) and paints them as an inline `↳ Routed from Router` divider via `src/tui/components/fork-divider.tsx` — distinct from the AssistantMessage frame so the marker reads as structural, not as content.
