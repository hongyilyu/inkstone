...
Tools without a registered baseline run unsandboxed (pi-coding-agent's own default), so `composeTools(info)` asserts that every composed tool either has a baseline or is explicitly listed as baseline-free because it does not touch filesystem paths.
