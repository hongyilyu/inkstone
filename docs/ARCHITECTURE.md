...
Ctrl+C follows the same scope rule: inside a dialog it's caught by `ui/dialog.tsx`'s `dialog_close`; at the session view it's caught by `app.tsx`'s `app_exit` (gated to `dialog.stack.length === 0`). The Prompt component installs a `useKeyboard` prelude (`prompt.tsx` + state machine in `prompt-ctrlc.ts`) that runs before the layout-level handler — both register on the same `renderer.keyInput` global-listener tier in mount order, child-first. Press 1 with text in the buffer → clear the input + `preventDefault` (no exit). Press 1 on empty buffer → arm a 5s "ctrl+c again to exit" hint in the prompt hints row + `preventDefault`. Press 2 within 5s → fall through (no `preventDefault`); the layout handler then performs `renderer.destroy() + process.exit(0)`. Typing into the prompt disarms. Mirrors OpenCode's clear-on-text behavior (`prompt/index.tsx:1025-1042`) plus Inkstone's existing Esc-double-tap idiom for the empty-prompt branch. Esc-Esc remains the sole agent-abort path; Ctrl+C never interrupts a streaming turn. Important dispatch detail: OpenTUI's global-listener tier does NOT short-circuit on `defaultPrevented`; what stops the layout handler on press 1 is its own explicit `if (evt.defaultPrevented) return` guard at `use-layout-keybinds.ts:35` — the same convention `ui/dialog.tsx` and `prompt-autocomplete.tsx` use.
