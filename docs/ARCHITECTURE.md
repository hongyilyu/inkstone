...
- `composeSystemPrompt(info)` — builds the full system prompt as five non-empty sections joined by blank lines, in order: the env block (`composeEnvBlock` — `<env>Today's date: YYYY-MM-DD</env>`, always present; pins the model's "today" against frontmatter-date inference drift), the workspace block (`composeWorkspaceBlock` — non-empty when the agent's overlay has any write/edit rule), the commands block (when `info.commands` has any entry with a `description`), `BASE_PREAMBLE` (empty today), and `info.buildInstructions()`. `buildInstructions` is nullary. Called once at `createSession` and again on `Session.selectAgent` (empty-session agent swap); not on every turn — `state.systemPrompt` stays byte-stable for the session's lifetime so Anthropic `cache_control` / Bedrock `cachePoint` prefixes hit. See D9's stability invariant. The env date is captured at compose time, so a session that crosses local midnight retains the original date until the next agent swap (acceptable trade-off vs. invalidating the cache prefix daily; matches OpenCode's `session/system.ts`).
...
Freeform text typed on the open page (no leading slash, no Tab pick) lands on the router by default (`DEFAULT_AGENT_NAME = "router"`, named explicitly so reordering `AGENTS` doesn't change the default). The router's first response calls `dispatch({ agent: "reader" | "knowledge-base" })`; pi-agent-core fires `tool_execution_end` and the reducer's `applyDispatchResult` handler:

1. Reads `agent` from the tool result + the first user-role message in `store.messages` (find-by-role, not index 0 — by the time `tool_execution_end` fires, `messages[0]` could be a synthetic assistant shell appended on `message_start`).
2. Calls `forkSession({ parentId, targetAgent, seedMessages })` (the persistence verb in `sessions.ts`) to atomically write the child session row, a forked-from marker (`parts.type = "fork"`), and the seeded user message into both `messages` and `agent_messages`.
3. Stashes the child session id on `sessionState.pendingDispatchChildId` and waits for the router's `agent_end` event. `handleAgentEnd` reads the stash and schedules the resume via `setTimeout(0)` — a *macrotask*, not a microtask. The macrotask boundary is load-bearing: pi-agent-core's `finishRun()` clears its `activeRun` only AFTER all `agent_end` listeners settle, so a microtask would still see `signal` truthy and `clearSession`'s busy-guard would take the async-abort branch (the void caller can't await; messages stay populated; `selectAgent` then throws "Agent is fixed for the lifetime of a session"). The macrotask runs after `finishRun()` completes, so `clearSession` takes the synchronous `agent.reset()` branch and the resume succeeds. The macrotask body wraps every store mutation in `batch()` so the swap renders as a single frame; `agent.continue()` fires after the batch to kick off the child agent's first turn (see `applyDispatchResult` in `reducer.ts` for the full timing rationale).

The TUI conversation renderer detects fork-marker rows (`role === "assistant"` + single `parts.type === "fork"` part) and paints them as an inline `→ Routing to <Target>` divider via `src/tui/components/fork-divider.tsx` — distinct from the AssistantMessage frame so the marker reads as structural, not as content. Layout: the seeded user message renders ABOVE the divider (the parent agent received it; the user bubble's accent reflects the parent agent via the per-message `agentName` stamp that `forkSession` writes); the divider sits below announcing the routing transition; the child agent's reply streams under the divider.

See ADR 0014 (fork as session primitive) and ADR 0015 (fork-marker discriminant) for the persistence-layer rationale.
...
- `helpers.prompt(text, opts?: PromptOptions)` starts an LLM turn. The first arg is what pi-agent-core (and in turn pi-ai) hands to the LLM. The options bag carries (1) `displayParts` — replaces the user bubble's rendered parts without changing what reaches the model (reader's `/article` uses this to inline the full article in `text` while rendering a compact "short prose + file chip" bubble via `displayParts = [text "Read this article.", file text/markdown <vault-relative>]`; commands that omit it get the default `[{ type: "text", text }]` shape) — and (2) `title` — declares the session title at dispatch time, persisted verbatim, skipping the LLM title task. Used when the command already knows the session's identity (reader's `/article` passes `frontmatter.title || filename-stem`; KB's `/ingest` and `/lint` pass `Verb · YYYY-MM-DD`). Capped at `MAX_TITLE_CHARS` — same bound the LLM-cleaned path enforces. See § Session-title generation for the bypass plumbing.
...
| `panel_close` | `components/session-list.tsx` — closes the session panel (ESC). `SessionList` calls `command.suspend()` on mount and `command.resume()` on cleanup so Ctrl+N inside the panel reaches `select_down` (move selection) instead of re-firing `session_list` (toggle close). |
...
`ctrl+n` is both `session_list` (global, opens the panel) and one alternate of `select_down` (panel-local, navigates inside the open panel). `SessionList` calls `command.suspend()` on mount and `command.resume()` on cleanup, mirroring the autocomplete dropdown — global `session_list` dispatch is dormant while the panel is open, so Ctrl+N inside reaches the panel-local `useKeyboard` for `select_down` only. Ctrl+P inside the panel reaches `select_up` for the same reason.
...
Ctrl+C follows the same scope rule: inside a dialog it's caught by `ui/dialog.tsx`'s `dialog_close`; otherwise it's caught by `useLayoutKeybinds`'s `app_exit` (gated to `dialog.stack.length === 0`). The layout handler implements two-stage clear/arm/exit semantics: press 1 with text clears, press 1 on empty arms a 5s "again to exit" hint, press 2 within the window exits. Esc-Esc remains the sole agent-abort path; Ctrl+C never interrupts a streaming turn.

The Prompt component publishes its decision callbacks via `LayoutContext`'s `setCtrlCBridge` so the single `useKeyboard` registration for `app_exit` (in `useLayoutKeybinds`) can read them back via `getCtrlCBridge()`. State machine in `src/tui/components/prompt-ctrlc.ts`; full design rationale (why a bridge instead of a second `useKeyboard`, what the bridge being null means) in [`./LAYOUT-CONTEXT.md`](./LAYOUT-CONTEXT.md) § PromptCtrlCBridge.
...
`sessions.title` is non-null. `createSession({ agent })` initializes it to a human-readable default (`"New session - <ISO timestamp>"`) so every session has a stable label immediately; on the first user prompt of a session, the title is settled via one of two paths:

1. **Command-declared (no LLM round-trip).** When `helpers.prompt(text, { title })` carries an `opts.title`, `promptAction`'s `applyExplicitSessionTitle` persists the trimmed title verbatim and the LLM title task is skipped. Used by reader's `/article` (`frontmatter.title || filename-stem`) and KB's `/ingest` / `/lint` (`Verb · YYYY-MM-DD`) — any verb whose session identity is knowable at dispatch time. Same persist-first shape as the LLM path.
2. **LLM-generated (background).** When no `opts.title` is set, the LLM title task is scheduled via `scheduleSessionTitleTask` (`tui/context/agent/actions/session-title.ts`) and `agentSession.actions.prompt(text)` continues without awaiting it. The generated title is persisted through `updateSessionTitle(tx, sessionId, title)` first, then mirrored into `store.sessionTitle` only if `currentSessionId` still points at the same row. Two callers schedule the task — they are exactly the two sites where a "first user message becomes a real session" event happens: `promptAction` (plain-chat first message) and the routing seam's `agent_end` macrotask (router-routed child sessions, after `resumeSession` runs so `modelProvider`/`modelId` already reflect the child agent for `resolveTitleModel`'s precedence chain). One shared helper, no drift.

Both paths share `MAX_TITLE_CHARS` (exported from `backend/agent/session-title.ts` so the explicit-title branch in `tui/context/agent/actions/prompt.ts` and the LLM-cleaning branch can't drift). Title generation in `backend/agent/session-title.ts`: resolve `config.sessionTitleModel` first, provider-local `titleModelId` second, active chat model last; pass the LLM-facing prompt text (or, for sessions with `displayParts`, a flattened "text + filename-stem" string built by `buildTitlePrompt`) capped at 4000 chars; call `completeSimple()` with `transport: "sse"` so Codex title calls do not touch the WebSocket cache/debug counters, `reasoning: "minimal"` to suppress thinking tokens on reasoning models, and a structured system prompt (task/rules/examples blocks ported from OpenCode's `title.txt`, trimmed to Inkstone's scope); clean output by stripping `<think>...</think>`, taking the first non-empty line, removing wrapping quotes, and capping to `MAX_TITLE_CHARS`. Failures are non-fatal; persistence failures use the same toast path as other session writers.
...
`panel_close: "escape"` is the dismiss keybind; only Esc closes the panel. Ctrl+N inside the open panel navigates down via `select_down`, and Ctrl+P navigates up via `select_up` — second-press-of-open-key navigates rather than toggles closed, matching the command palette's Ctrl+P pattern. The dispatch wiring (`command.suspend()` on mount + `command.resume()` on cleanup) is documented under § Collision safety.

The panel's scroll viewport follows the keyboard selection: `moveTo()` calls a `syncScroll()` helper that locates the freshly-selected row by `id` (each `SessionListItem` stamps `id={row.id}` on its outer box) and `scrollBy`s when the row is outside `[0, scroll.height)`. Ported from `DialogSelect`'s `moveTo` (`src/tui/ui/dialog-select.tsx:171-193`); two reference branches are intentionally omitted because they have no caller: (a) the `center` parameter for filter-driven jumps (the panel has no filter), and (b) the `current` prop's mount-time scroll-to (initial selection is always index 0 with scroll already at top). The wrap-to-top clamp (`scrollTo(0)` when `selected === 0`) matches the reference and snaps the viewport flush to the top after an upward wrap.
