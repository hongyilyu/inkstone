...
Ctrl+C follows the same scope rule: inside a dialog it's caught by `ui/dialog.tsx`'s `dialog_close`; otherwise it's caught by `useLayoutKeybinds`'s `app_exit` (gated to `dialog.stack.length === 0`). The layout handler implements two-stage clear/arm/exit by consulting a module-scoped bridge (`app.ts`'s `setPromptCtrlCBridge` / `getPromptCtrlCBridge`) that the Prompt component publishes on mount with four callbacks: `decide()` (returns the next action via `prompt-ctrlc.ts`'s pure state machine), `clear()` (wipes the textarea), `arm()` (sets a signal driving the "ctrl+c again to exit" hint and starts a 5s reset timer), and `disarm()` (clears both). Press 1 with text → `clear` + `preventDefault`. Press 1 on empty → `arm` + `preventDefault`. Press 2 within 5s → `disarm` then `renderer.destroy() + process.exit(0)`. Typing into the prompt disarms via a `createEffect` in `prompt.tsx`. When the bridge is null (no-provider boot fallback, approval / suggestion panels — Prompt isn't mounted) the layout handler exits immediately, matching pre-existing behavior on those surfaces. The bridge pattern (rather than a second `useKeyboard` in `prompt.tsx`) is required because OpenTUI's `EventEmitter` dispatches global listeners in registration order, so the layout's `onMount` (parent) registers before the prompt's (child) — a prompt-level handler would fire AFTER the layout's exit and never get a chance to intercept. Mirrors OpenCode's clear-on-text behavior (`prompt/index.tsx:1025-1042`) plus Inkstone's existing Esc-double-tap idiom on the empty-prompt branch. Esc-Esc remains the sole agent-abort path; Ctrl+C never interrupts a streaming turn.
